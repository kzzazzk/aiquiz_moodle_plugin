...
$event = \mod_aiquiz\event\course_module_instance_list_viewed::create([
...
if (empty($aiquizs)) {
    notice(get_string('no$aiquizinstances', 'mod_aiquiz'), new moodle_url('/course/view.php', ['id' => $course->id]));
}
...
foreach ($aiquizs as $aiquiz) {
    if (!$aiquiz->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/aiquiz/view.php', ['id' => $aiquiz->coursemodule]),
    ...
    if ($course->format == 'weeks' || $course->format == 'topics') {
        $table->data[] = [$aiquiz->section, $link];
    } else {
        $table->data[] = [$link];
    }
}
