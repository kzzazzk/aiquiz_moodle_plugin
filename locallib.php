...
function quiz_set_grade($newgrade, $quiz) {
    ...
    $oldgrade = $quiz->grade;
    $quiz->grade = $newgrade;
    $value = required_param('cmid',PARAM_INT);
    $aiquiz_link = $DB->get_field('course_modules','instance',['id' => $value]);
    $quiz_id = $DB->get_field('aiquiz','quiz_id',['id' => $aiquiz_link]);
    ...
    $DB->set_field('quiz', 'grade', $newgrade, array('id' => $quiz_id));
...
function quiz_add_quiz_question($questionid, $quiz, $page = 0, $maxmark = null) {
    ...
    $value = $DB->get_field('aiquiz','quiz_id',  ['id' => $quiz->id]);
    $cm = get_coursemodule_from_instance('quiz', $value);
...
function create_question($questiontext, $qtype = 'multichoice', $categoryid = 1) {
    global $DB;
...
function add_question_to_quiz($quizid, $questionid) {
    global $DB;
    ...
    $slot->maxmark = 1.0;
    $slot->slot = $DB->count_records('quiz_slots', array('quizid' => $quizid)) + 1;
