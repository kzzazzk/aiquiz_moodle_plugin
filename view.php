...
$event->trigger();
$timenow = time();
$value = required_param('id',PARAM_INT);
$aiquiz_link = $DB->get_field('course_modules','instance',['id' => $value]);
$quiz_id = $DB->get_field('aiquiz','quiz_id',['id' => $aiquiz_link]);
$quiz_instance = $DB->get_field('course_modules','id',['instance' => $quiz_id]);

$quizobj = quiz::create($quiz_id, $USER->id);
$accessmanager = new quiz_access_manager($quizobj, $timenow,
    has_capability('mod/aiquiz:ignoretimelimits', $context, null, false));
...
$viewobj->canedit = has_capability('mod/aiquiz:manage', $context);
